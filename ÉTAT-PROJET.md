...
- **PRIX AU M² PAR QUARTIER — source de vérité unique : `lib/quartiers.ts`** (2026-09-15). Les 7 médianes DVF publiables, la médiane communale, les typologies, le loyer médian communal, plus les helpers (`ecartCommune`, `surfacePourBudget`, `honorairesVente`, formatage FR). **Aucune page n'écrit un prix en dur** : pages quartiers, `/estimation` et le tableau `components/seo/quartier-prix.tsx` lisent tous ce fichier. Pour la mise à jour DVF 2026 : un seul endroit à changer (méthode consignée dans `SEO-JOURNAL.md`). Quartier Saint-Jean volontairement non publié (échantillon trop mince).
- **Pages quartiers — contenu de recherche** (2026-09-15) : 350-650 mots → **1 295 à 1 443 mots rendus** par page, un angle distinct chacune (Gratte-Ciel = volume de transactions ; Charpennes = écart prix d'annonces / ventes réelles + rendement calculé ; Cusset = surface par euro), tableau des 7 quartiers, `updated` visible, **FAQ de 5 Q/R + JSON-LD `FAQPage` générés depuis le même tableau**. Maillage `/estimation` → quartiers posé (6 liens sortants, il n'y en avait aucun) + maillage latéral entre quartiers. **L'outil d'estimation n'a pas été touché** (section statique hors `<EstimationForm />`).
...
- **Articles blog 2 & 9** : ajouter les vrais prix €/m² par quartier (Villeurbanne) dans `lib/blog.ts` — actuellement formulé sans chiffres inventés. **Les chiffres existent désormais dans `lib/quartiers.ts`**, il n'y a plus rien à attendre ni à calculer pour lever ce point.
